  ...
  describe('#update', function() {
    it('.git/hooks/pre-commit should symbol link to bin/pre-commit', sinon.test(function(done) {
      ...
      env.Feature.makeScaffold(env.testScaffoldRoot).then(function() {
        require('fs').readlink(path.join(env.testScaffoldRoot, '.git/hooks/pre-commit'), function(err, target) {
    ...
    after(function() { // TODO
      ...
      require('fs').unlinkSync(path.join(env.testScaffoldRoot, '.git/hooks/pre-commit'));
    });
  });
});
