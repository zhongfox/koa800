 ...
 * @returns {*} 返回YieldableNetAction或者faultTolerance的返回值
 */
let netFT = function* (YieldableNetAction, faultTolerance, options) {
  ...
  let timeoutPromise = new Promise(function (resolve, reject) {
    timer = setTimeout(function() {
      var err = new app.errors.NetFTError('netFT Timeout Error');
      reject(err);
    }, timeout);
  });
  ...
  try {
    ...
    if (type === 'http' && result.statusCode >= 400) {
      throw (new app.errors.NetFTError('netFT http status: ' + result.statusCode));
    }
  }
  catch(e) {
